...
public class BricksManager
{
  ...
  private final static int MAX_BRICKS_LINES = 30;
...
//	  char[] tileList = new char[ALPHABET.length()];
	  
	  for (int i = 0; i < ALPHABET.length(); i++)
		  list.put(ALPHABET.charAt(i), i);
	  ...
	  for(int x=0; x < line.length(); x++) {
		  ...
		  if (ch == ' ')   // ignore a space
			  continue;
		  if( x == '*')
			  endOfMap = new Brick(list.get(ch), x, lineNo);
		  bricksList.add( new Brick(list.get(ch), x, lineNo) );
	  }
  }  // end of storeBricks()
  ...
  */
  ...
  //show if the brick map ended or started
  public boolean end(){
	  if(xMapHead == (width - 1 * pWidth) * -1)
		  return true;
	  return false;
  }
